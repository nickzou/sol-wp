...
const setupCssOption = async ({registerAssets, answers, npmPackages, packageScripts, watchScripts, devScripts, prettierConfigOptions}:SetupCss) => {
    const cssOptions = {
    ...
    try {
        ...
        if(optionFunction) {
            await optionFunction({registerAssets, answers, npmPackages, packageScripts, watchScripts, devScripts, prettierConfigOptions});
        }
    } catch (error) {
        throw error;
    }
